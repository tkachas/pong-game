...
let goalSound = new Howl({
  src: ["./content/goal.wav"],
});
let wallHitSound = new Howl({
  src: ["./content/wall-hit.mp3"],
});
...
function ballMove() {
  if (
    ball.y >= rightRocket.x - parseInt(ballStyle.width) &&
    ball.y <= rightRocket.x + parseInt(rightStyle.height) &&
    ball.x + ballSpeed >= 750
  ) {
    if (
      ball.y >= rightRocket.x - (parseInt(ballStyle.width) + 1) &&
      ball.y <= rightRocket.x + 1
    ) {
      movement["down"] = false;
      movement["up"] = true;
    }
    if (
      ball.y >= rightRocket.x + (parseInt(rightStyle.height) - 1) &&
      ball.y <= rightRocket.x + parseInt(rightStyle.height)
    ) {
      movement["down"] = true;
      movement["up"] = false;
  ...
  if (
    ball.y >= leftRocket.x - parseInt(ballStyle.width) &&
    ball.y <= leftRocket.x + parseInt(leftStyle.height) &&
    ball.x - ballSpeed <= 30
  ) {
    if (
      ball.y >= leftRocket.x - (parseInt(ballStyle.width) + 1) &&
      ball.y <= leftRocket.x + 1
    ) {
      movement["down"] = false;
      movement["up"] = true;
    }
    if (
      ball.y >= leftRocket.x + (parseInt(leftStyle.height) - 1) &&
      ball.y <= leftRocket.x + parseInt(leftStyle.height)
    ) {
      movement["down"] = true;
      movement["up"] = false;
...
function startAgain() {
  ...
  rightRocket.element.style.top =
    (500 - parseInt(rightStyle.height)) / 2 + "px";
  ball.element.style.top = ball.y + "px";
  ball.element.style.left = ball.x + "px";
  movement["up"] = false;
  movement["down"] = false;
